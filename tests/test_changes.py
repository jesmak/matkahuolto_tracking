...
from custom_components.matkahuolto_tracking.changes import (
    EVENT_DELIVERED,
    EVENT_MOVED,
    EVENT_NEW,
...
def test_the_first_update_is_not_news() -> None:
    assert detect_changes(None, [package("MH0001", 3)]) == []


def test_a_package_that_was_not_there_before() -> None:
    assert types(detect_changes([], [package("MH0001", 1)])) == [EVENT_NEW]


def test_a_status_of_its_own_gets_an_event_of_its_own() -> None:
    before = [package("MH0001", 3), package("MH0002", 4)]
    after = [package("MH0001", 5), package("MH0002", 0)]
    assert types(detect_changes(before, after)) == [EVENT_READY_FOR_PICKUP, EVENT_DELIVERED]


def test_any_other_step_is_a_package_moving() -> None:
    before = [package("MH0001", 2)]
    assert types(detect_changes(before, [package("MH0001", 3)])) == [EVENT_MOVED]


def test_a_new_event_without_a_new_status_is_a_package_moving() -> None:
    before = [package("MH0001", 3, "2026-09-16T05:30:00Z")]
    after = [package("MH0001", 3, "2026-09-16T07:15:00Z")]
    assert types(detect_changes(before, after)) == [EVENT_MOVED]


def test_a_package_that_did_not_change_is_not_news() -> None:
    before = [package("MH0001", 3)]
    assert detect_changes(before, [package("MH0001", 3)]) == []


def test_several_changes_in_one_update_are_several_events() -> None:
    before = [package("MH0001", 3), package("MH0002", 4)]
    after = [package("MH0001", 5), package("MH0002", 0), package("MH0003", 1)]
...
def test_an_event_carries_the_package_it_happened_to() -> None:
    [change] = detect_changes([], [package("MH0001", 5)])
