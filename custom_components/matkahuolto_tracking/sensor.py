...
from .shipments import STATUS_DELIVERED, STATUS_READY_FOR_PICKUP
...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            MatkahuoltoSensor(coordinator),
            MatkahuoltoCountSensor(coordinator, "on_the_way", lambda status: status != STATUS_DELIVERED),
            MatkahuoltoCountSensor(coordinator, "ready_for_pickup", lambda status: status == STATUS_READY_FOR_PICKUP),
        ]
    )
...
class MatkahuoltoCountSensor(CoordinatorEntity[MatkahuoltoCoordinator], SensorEntity):
    ...
    def __init__(self, coordinator: MatkahuoltoCoordinator, key: str, counts: Callable[[Any], bool]) -> None:
        super().__init__(coordinator)
        entry = coordinator.config_entry
        username = entry.data[CONF_USERNAME]
        self._counts = counts
        ...
        self._attr_unique_id = f"matkahuolto_{username}_{key}"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, entry.entry_id)},
            name=f"Matkahuolto ({username})",
            manufacturer="Matkahuolto",
